...
class CharDecoder(nn.Module):
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        batchsize = dec_hidden[0].size(1)
        current_char = torch.tensor([[startidx]*batchsize],device=device)
        ...
        output = torch.cat(output).t().tolist()
        for word in output:
            word_result=""
            for idx in word:
                if idx==endidx:
                    break
                word_result += self.target_vocab.id2char[idx]
            decodedWords += [word_result]
