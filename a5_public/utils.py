    ...
    for sentence in sents:
        ...
        for word in sentence:
            ...
            if len(word)<max_word_length:
                num_pad_tokens = max_word_length-len(word)
                for character in word:
                    tmpword.append(character)
                for idx in range(num_pad_tokens):
                    tmpword.append(char_pad_token)
            else:
                for idx in range(max_word_length):
                    tmpword.append(word[idx])
            tmpsentence.append(tmpword)
        for idx in range(longest_length-len(sentence)):
            tmpsentence.append([char_pad_token]*max_word_length)
        sents_padded.append(tmpsentence)
    ### END YOUR CODE
...
def pad_sents(sents, pad_token):
    ...
    for sentence in sents:
        tmpsentence=[]
        for word in sentence:
            tmpsentence.append(word)
        
        for idx in range(longest_length-len(sentence)):
            tmpsentence.append(pad_token)
