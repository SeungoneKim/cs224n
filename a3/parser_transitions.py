...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        ###         3. Right Arc
        if transition=="S":
            moving_word = self.buffer.pop(0)
            self.stack.append(moving_word)

        elif transition=="LA":
            moving_word = self.stack.pop(len(self.stack)-2)
            self.dependencies.append((self.stack[-1],moving_word))
        
        else:
            moving_word = self.stack.pop(len(self.stack)-1)
            self.dependencies.append((self.stack[-1],moving_word))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        transition_prediction = model.predict(minibatch)

        for i,pp in enumerate(minibatch):
            pp.parse_step(transition_prediction[i])
